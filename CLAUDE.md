...
- **iPhone Duo (Xcode 27.1 / iOS 27.1) has two integrated panels and boots folded.** `primary` (screen 1, 1398×2034, lit) and `primary-1` (screen 3, 2007×2853, which the guest turns off). "Largest portrait port" bound the dark one and `0x32` addressed its digitizer (the slot belongs to the last built-in panel created). The phone plane now binds the Connected Screen named `primary` and, only when `IntegratedPanels.several`, addresses that panel's own `0x40000000 | screenId` registration — one `simctl io enumerate` on the Duo, nothing new on single-panel devices. Nothing in this beta unfolds it: no `simctl` / `devicectl` / Device Hub control, and the guest's hinge path is an entitled `SBSDisplayToolService`. See `docs/features/iphone-duo.md`.
