...
- **iPhone Duo (Xcode 27.1 / iOS 27.1) has two integrated panels and the hinge decides which is lit.** `primary` (screen 1, 1398×2034, `phone15`) and `primary-1` (screen 3, 2007×2853, `phone14`); it boots folded. Device Hub's bottom-bar pose picker folds/unfolds it (closed ≈3°, open ≈130°, SpringBoard goes landscape on its own when open) over CoreDevice's UniversalHID, which baguette cannot speak — so baguette **reads** the angle with `devicectl device motion hinge-angle` (`DevicectlHinge`, first sample ~0.3 s, only when `IntegratedPanels.several`) and binds the lit panel: framebuffer, digitizer (`0x40000000 | screenId`; `0x32` is a slot owned by the last built-in panel created, i.e. the unfolded one), chrome, tap space, AX point size, and the page's rotation. `0x32` on the Duo is *not* the cover. Powering `primary-1` on via `screenConfig` lights nothing; powering it *off* crash-loops SpringBoard — leave screen power alone. **The page draws the Duo as Device Hub does: Apple's `V68.usdz` (in `DeviceKit.framework`'s `CoreDevicePopDeviceKitExtension` plug-in) on the RealityKit pipeline, posed by the hinge (`FoldPose`) — the flat chromes have no sides, hinge or keys.** The guest's interface orientation is not readable from the host (`simctl io enumerate` says `Ambiguous` for the unfolded panel); the page's rotate button sets it. `devicectl`'s hinge stream can go silent after a SpringBoard restart. See `docs/features/iphone-duo.md`.
