...
- **Xcode 27's Device Hub shadows the legacy input surface.** Its guest daemon `dtuhidd` publishes `com.apple.coredevice.dtuhidd.active = 1`, and backboardd's `SimulatorHID` disconnects the legacy Indigo services (`ScreenTouchService`, `MainScreenButtonsService`, `ExternalKeyboardService`) in response; the iOS 27 runtime then re-adds the *same dead* `IOHIDServiceRef` on the next Indigo event and never retries. Touch survives only if SimulatorHID had already seen an Indigo event, or was born with dtuhidd active (a ~100 ms boot race); buttons never survive. `InputSurface.reclaim` — `notifyutil -s … 0` **then** `launchctl kickstart -k system/com.apple.backboardd` in the guest, in that order — is the verified repair; `baguette boot` runs it, `baguette heal` runs it on demand. Don't try to prevent it during boot: SimulatorHID reacts to state *changes*, so forcing `0` early still produces the fatal connect→disconnect. See `docs/features/device-hub.md` and #77.
...
- **A HID target is only ever a constant some create-service message registered — never a computed number.** `SimHIDVirtualServiceManager` keeps registered services in a dictionary and **throws** on an unknown target, killing `backboardd` and SpringBoard with it (it presents as the simulator spontaneously rebooting). The guest names the valid set when it dies: `'Encountered HID event with unexpected target 1073741826 not in known targets: (50, 13, 11, 53, 51, 302, 300, 1, 14, 60, 12, 100, 54, 1073741825, 301)'` — `50` = `0x32` phone, `53` = `0x35` pointer, `54` = `0x36` mouse, and the plain `1` is CarPlay. **`IndigoHIDTargetForScreen` is a trap**: it is a real SimulatorKit export returning `0x40000000 | screenId`, which looks like the answer and is a target nothing registered (`0x40000002` above is screen 2). CarPlay is a *fixed* plain `1` because `IndigoHIDMessageToCreateCarPlayService` hardcodes `1` into the target slot at `[0x40]` and the guest keys its registry on that raw value, exactly as its siblings hardcode theirs. `1073741825` (`0x40000001`) is the near-miss worth knowing — `1` with a `0x40000000` flag invented to match `IndigoHIDTargetForScreen`; it is in fact **screen 1's own digitizer**: `createDigitizerForTargetID:withDisplayUID:isBuiltIn:` registers every *Integrated* screen's `ScreenTouchService` under `0x40000000 | screenId` (the mask bit is what the create message must carry), so it never crashed, it just delivered CarPlay's touches to the phone. `0x32` is not a digitizer but a **slot** the same call fills for a built-in panel — last created wins — which is why `IndigoHIDTouchTarget.panel(screenId:)` exists and is fed only screen ids Connected Screens lists as Integrated. Only targets in that published list are ever valid, which is what `BAGUETTE_CARPLAY_TARGET` will accept. That service must also be **created** before use — `warmServices` did pointer and mouse but not CarPlay; its one byte is `hasTouchScreen`, a BOOL, not a screen index (from Simulator.app's `[[self starkConfig] hasTouchScreen]`; "Stark" is Apple's codename for CarPlay). Opcode `[0x30]`: 3 pointer / 5 mouse / 7 CarPlay / 8 remove-CarPlay. See [`docs/features/companion-screens.md`](docs/features/companion-screens.md).
- **iPhone Duo (Xcode 27.1 / iOS 27.1) has two integrated panels and the hinge decides which is lit.** `primary` (screen 1, 1398×2034, `phone15`) and `primary-1` (screen 3, 2007×2853, `phone14`); it boots folded. Device Hub's bottom-bar pose picker folds/unfolds it (closed ≈3°, open ≈130°, SpringBoard goes landscape on its own when open) over CoreDevice's UniversalHID, which lands in the guest daemon `dtuhidd` as vendor-defined IOHIDEvents (page `0xFF61`, usage `0x5B`, a keyed record with `source: hinge-slider-control`, `value: <degrees>`) on its `avpCustom` virtual HID service. baguette **drives** the hinge by reproducing exactly that from a guest-side executable, `HingeControl serve` (`Injected/HingeControl`, private `HID.framework`'s `HIDVirtualEventService`; `GuestHingeMotor`, `Hinge.fold(to:over:)`, `baguette hinge`, `POST /hinge`, `docs/features/hinge.md`), and **reads** the angle with `devicectl device motion hinge-angle` (`DevicectlHinge`, first sample ~0.3 s, only when `IntegratedPanels.several`) and binds the lit panel: framebuffer, digitizer (`0x40000000 | screenId`; `0x32` is a slot owned by the last built-in panel created, i.e. the unfolded one), chrome, tap space, AX point size, and the page's rotation. `0x32` on the Duo is *not* the cover. Powering `primary-1` on via `screenConfig` lights nothing; powering it *off* crash-loops SpringBoard — leave screen power alone. **The page draws the Duo as Device Hub does: Apple's `V68.usdz` (in `DeviceKit.framework`'s `CoreDevicePopDeviceKitExtension` plug-in) on the RealityKit pipeline, posed by the hinge (`FoldPose`) — the flat chromes have no sides, hinge or keys.** **The Duo's hardware keys do not take the legacy Indigo press** — it reaches backboardd on a touchscreen service and SpringBoard ignores it; Device Hub presses volume (`0x0C`/`0xE9`,`0xEA`), power (`0x0C`/`0x30`) and camera control (`0xFF00`/`0x66`) as keyboard events on `dtuhidd`'s `mainScreenButtons` service (page `0x0B`/`0x01`, built-in), so `HingeControl button` does the same and `FoldableInput` routes a foldable's buttons there (`DeviceKeys`). The page's rotate button turns the book (`InterfaceRoll`) and tells the guest; nothing reads the guest's orientation back (`simctl io enumerate` says `Ambiguous` for a dark or turning panel; backboardd's `OrientationDevice` log has it, unused). `devicectl`'s hinge stream can go silent after a SpringBoard restart. See `docs/features/iphone-duo.md`.
