...
struct DevicePresentation: Equatable, Sendable {
    ...
    let nativeOrientation: Double?
    let bezelPNG: Data?
    /// The screen id of the panel this describes, for a device with more
    /// than one. Its presence tells a caller the answer moves with the
    /// hinge and must not be cached; a single-display device omits it.
    var panel: UInt32? = nil
    /// Every integrated panel of a multi-display device, in canvas order.
    var panels: [DisplayPanel] = []

    /// This device, presenting on `panel`. DeviceKit draws a foldable
    /// closed, so its art and screen rect describe the main panel only;
    /// any other panel is shown bare, at its own size, in the raw
    /// framebuffer frame every coordinate here is expressed in.
    func presenting(on panel: DisplayPanel, of panels: DisplayPanels) -> DevicePresentation {
        var presentation = self
        if panel != panels.main {
            let size = Size(width: Double(panel.pointSize.width), height: Double(panel.pointSize.height))
            presentation = DevicePresentation(
                style: .frameless,
                identifier: nil,
                formFactor: formFactor,
                viewport: size,
                screen: Rect(origin: Point(x: 0, y: 0), size: size),
                screenScale: panel.scale,
                screenCornerRadii: .zero,
                nativeOrientation: nativeOrientation,
                bezelPNG: nil
            )
        }
        presentation.panel = panel.screenID
        presentation.panels = panels.panels
        return presentation
    }
    ...
    var snapshot: DevicePresentationSnapshot {
        ...
        if let panel {
            json["panel"] = Int(panel)
            // A foldable streams one canvas of these, side by side in this
            // order and top-aligned, each `floor(pixels / scale)` rounded
            // down to even; a host lays the canvas out from them.
            json["displays"] = panels.map { panel -> [String: Any] in
                [
                    "screenId": Int(panel.screenID),
                    "pixelWidth": Int(panel.pixelSize.width),
                    "pixelHeight": Int(panel.pixelSize.height),
