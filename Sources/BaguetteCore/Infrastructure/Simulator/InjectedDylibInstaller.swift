...
enum InjectedDylibInstaller {
    ...
    ///   3. Sidecar `Baguette_BaguetteCore.bundle` (or the historical
    ///      `Baguette_Baguette.bundle`) next to the executable.
        ...
        for _ in 0..<6 {
            let candidates = [
                url.appendingPathComponent(
                    "Sources/BaguetteCore/Resources/\(dylib.name)/\(dylib.fileName)"),
                url.appendingPathComponent(dylib.sourceTreePath),
            ]
