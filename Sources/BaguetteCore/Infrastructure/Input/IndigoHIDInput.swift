...
final class IndigoHIDInput: Input, DisplayAddressable, @unchecked Sendable {
    ...
    func button(_ button: DeviceButton, duration: Double) -> Bool {
        ...
        case .appSwitcher:
            ...
            let downA = pressHomeOrLock(.home, holdUs: holdUs, on: c)
            usleep(150_000)
            let downB = pressHomeOrLock(.home, holdUs: holdUs, on: c)
            return downA && downB
    ...
    func key(_ key: KeyboardKey, modifiers: Set<KeyModifier>, duration: Double) -> Bool {
        ...
        var heldModifiers: [KeyModifier] = []
        var heldKey = false
        defer {
            if heldKey, let up = kfn(target, key.hidUsage.page, key.hidUsage.usage, 2) {
                send(message: up, to: c)
            }
            for m in heldModifiers.reversed() {
                if let up = kfn(target, m.hidUsage.page, m.hidUsage.usage, 2) {
                    send(message: up, to: c)
                }
            }
        }
