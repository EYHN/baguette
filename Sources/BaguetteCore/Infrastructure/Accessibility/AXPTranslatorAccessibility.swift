...
final class AXPTranslatorAccessibility: Accessibility, @unchecked Sendable {
    ...
    func adjust(request: String) throws -> String {
        ...
        guard let result: String = try withAXPContext({ ctx in
    ...
    private func withAXPContext<T>(
        ...
        // the previous request's expired token. Bind it BEFORE reading the
        // root frame, not just before walking its children.
        Self.stampElementTranslation(token: token, on: frontmostRoot)
        // A foldable's AX space is the lit panel's, which the hinge moves;
        // every other device's is its one screen. The panel comes from the
        // shared hinge reading (`ActiveDisplays`), which is cheaper than the
        // injected `litPanelPointSize` resolve and answers the same question.
        let panel = ActiveDisplays.shared.activePanel(udid: udid, device: device)
        let pointSize = panel?.pointSize ?? litPanelPointSize() ?? Self.devicePointSize(for: device)
        ...
        if ProcessInfo.processInfo.environment["SIMKIT_AX_DEBUG"] == "1" {
            log("[ax] geometry root=\(rootFrame) points=\(pointSize) orientation=\(String(describing: orientation))")
        }
        // A landscape AX root without authoritative direction is ambiguous.
        // Never manufacture a tap target by guessing left versus right.
        guard orientation != nil || rootFrame.width <= rootFrame.height else { return nil }
    ...
    private static func screenOrientation(_ device: NSObject) -> ScreenOrientation? {
        // Reuse the framebuffer descriptor path used by capture. Constructing
        // Simulator.app's SimDeviceScreen here requires its main queue and
        // can trap when an embedded host calls AX on a worker thread.
        ...
        // Missing orientation during rotation is not a missing framebuffer.
        // Do not refresh live IO ports and disrupt an active capture pipeline.
