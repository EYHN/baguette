...
enum HostProcess {
    struct Child: @unchecked Sendable {
        ...
        /// The write end of the child's stdin — only for a child spawned
        /// with `StandardInput.interactive`, whose owner feeds it for as
        /// long as it lives. `nil` otherwise.
        let input: FileHandle?

        init(pid: pid_t, output: FileHandle, input: FileHandle? = nil) {
            self.pid = pid
            self.output = output
            self.input = input
        }
    ...
    enum StandardInput {
        /// `/dev/null` — detached from any controlling terminal, so a
        /// SIGINT handed to the parent doesn't also kill the child via the
        /// foreground pgid before the parent's own handlers run.
        case null
        /// A one-shot payload, written off the calling thread and closed
        /// (EOF) once delivered.
        case payload(Data)
        /// A pipe kept open for the owner to `write` to line by line
        /// (`HingeControl serve`); closing `Child.input` delivers EOF.
        case interactive
    }
    ...
    static func spawn(
        ...
        try spawn(
            ...
            stdin: stdinData.map(StandardInput.payload) ?? .null,
    ...
    static func spawn(
        executable: URL,
        arguments: [String],
        stdin standardInput: StandardInput,
        workingDirectory: URL?,
        environment: [String: String]?
    ) throws -> Child {
        let pipe = Pipe()
        ...
        case .null: stdinPipe = nil
        case .payload, .interactive: stdinPipe = Pipe()
        }
        ...
        case .interactive:
            return Child(
                pid: pid, output: pipe.fileHandleForReading,
